        ...
        double vel, double acc_time, int blend_percent, bool fine_goal, const std::string &id)
{
    ...
    return (sct.send_script_str(
        id, TmCommand::set_joint_pos_PTP(angs, vel_pa, acc_time, blend_percent, fine_goal)) == RC_OK);
}
        ...
        double vel, double acc_time, int blend_percent, bool fine_goal, const std::string &id)
{
    int vel_pa = int(100.0 * (vel / _max_velocity));
    return (sct.send_script_str(
        id, TmCommand::set_tool_pose_PTP(pose, vel_pa, acc_time, blend_percent, fine_goal)) == RC_OK);
}
...
{
    return (sct.send_script_str(
        id, TmCommand::set_tool_pose_Line(pose, vel, acc_time, blend_percent, fine_goal)) == RC_OK);
}
