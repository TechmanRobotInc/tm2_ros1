...
def _gen_xacro():
    ...
    ###############################################################################################
    # example: generate a new_model file (macro.xxxooo.urdf.xacro), base on tm5s-norminal model
    # syntax : python3 modify_xacro.py original_model new_model
    # [key-in] original_model: tm5s  , [key-in] new_model: xxxooo
    # [key-in] shell cmd $ python3 modify_xacro.py tm5s xxxooo
    ###############################################################################################

    if len(sys.argv) < 3:
        print('Incorrect syntax! at least 2 parameters are required')
        print('You can try: python3 modify_xacro.py tm5s test')
        return
